...
async function handleFollowUp() {
    ...
    try {
        ...
        if (data.success) {
            // 加入系統回覆到歷史
            conversationHistory.push({ role: 'assistant', content: data.answer.replace(/<br\/?>/g, '\n') });
            ...
            sysLog.innerHTML = `<span class="opacity-50 text-[10px] block mb-1">[${new Date().toLocaleTimeString()}] SYSTEM_REPLY:</span><div class="font-mono">${data.answer}</div>`;
